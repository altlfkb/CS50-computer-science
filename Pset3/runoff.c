...
void tabulate(void)
{
    for (int l = 0; l < voter_count; l++) {
        if (candidates[preferences[l][0]].eliminated) {
            int somar = 0;
            do {
                somar ++; }
            while (candidates[preferences[l][somar]].eliminated) ;
            candidates[preferences[l][somar]].votes++;
        }
        else {
            candidates[preferences[l][0]].votes++;
        }
    }
    return;
}
